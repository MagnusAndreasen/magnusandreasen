<!DOCTYPE html>
<html lang="en">

  <head>
  <meta name="description" content="">
  <meta name="author" content="">
  <title>Portfolio</title>
  <?php include 'head.php';?>  
  </head>

  <body id="page-top">

    <!-- Navigation -->
    <a class="menu-toggle rounded-0" href="#">
      <i class="fas fa-bars"></i>
    </a>
   <?php include 'menu.php';?> 

    <!-- Portfolio -->
	<div class="jumbotron jumbotron-fluid bg-primary">
  		<div class="container text-center text-white">
    		<h1 class="display-4">All Projects</h1>
    		<p class="lead">Below you can see all of the projects I have done, both web based projects and design projects.<br><br>Click on the "See More" button to see details.</p>
  		</div>
	</div>
    <div class="mb-5">
      <div class="container">
		<div class="content-section-heading text-center">
		  <h3 class="mb-5">Web Based Projects</h3>
    	</div>
        <div class="row justify-content-center">
          <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/strongyou-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Strong You</h5>
   				<p class="card-text">Designed and developed a website for the danish fitness and health company Strong You! Made in Wordpress. Used mainly PHP and JavaScript for this project.</p>
    			<a href="strongyou.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
		  <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/strongyou-prototype-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Strong You Prototype</h5>
   				<p class="card-text">Designed and developed an app function for the danish fitness and health company Strong You! I used PHP, JavaScript and NoSql Firebase for this project.</p>
    			<a href="strongyouprototype.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
		  <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/hugo.fo-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Hugo.fo</h5>
   				<p class="card-text">Designed and developed a website for Guesthouse Hugo & Café Fjørðoy in the Faroe Islands! Made in Wordpress. Used mainly PHP and JavaScript for this project.</p>
    			<a href="hugo.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
		  <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/politikertweets-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Politician Tweets</h5>
   				<p class="card-text">Designed and developed a website that showes the latest tweets from danish politicians. This was a school project! Built from scratch with Bootstrap and PHP and has elements of JavaScript in it.</p>
    			<a href="strongyou.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
		  <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/cocktailguide-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Cocktail Guide</h5>
   				<p class="card-text">Designed and developed a cocktail recipe website as a school project! Built from scratch with Bootstrap and has elements of JavaScript and PHP in it.<br><br></p>
    			<a href="strongyou.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
		  <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/postitboard-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Post It Board</h5>
   				<p class="card-text">Designed and developed a Post It Board website as a school project! Used PHP and JavaScript to build the site. Used the NoSql database Firebase to store all data.<br><br></p>
    			<a href="strongyou.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
		  <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/nordlid-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Nordlið</h5>
   				<p class="card-text">Developed a page for the company Nordlið as part of a contest to become their next intern. I was among the 3 best contestants.<br><br></p>
    			<a href="strongyou.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
		  <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/portfolio-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Portfolio</h5>
   				<p class="card-text">Designed and developed a portfolio website as a school project! Used HTML, CSS and JavaScript to build the site. This is the first complete website I have built.</p>
    			<a href="strongyou.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
		  <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/lommeregner-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Calculator</h5>
   				<p class="card-text">Developed a calculator website as a school project! Used HTML, CSS and JavaScript to build the site.<br><br><br></p>
    			<a href="strongyou.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
        </div>
		<br>
		<hr>
		<br>
		<div class="content-section-heading text-center">
		  <h3 class="mb-5">Design Based Projects</h3>
    	</div>
		<div class="row justify-content-center">
          <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/cafemenu-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Café Fjørðoy Menu</h5>
   				<p class="card-text">Designed and developed a food menu and drinks menu for the café. The menu design was based on the website design. The menu was made with Adobe InDesign.</p>
    			<a href="strongyou.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
		  <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/hugo-poster-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Café Fjørðoy Posters</h5>
   				<p class="card-text">Designed and developed posters for the café front door, counter and outside the café. The design was based on the website design. The poster was made with Adobe InDesign.</p>
    			<a href="strongyou.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
		  <div class="card col-lg-3 mb-3 mx-3 rounded-0" style="width: 18rem;">
  			<img class="card-img-top rounded-0" src="img/aires-small.png" alt="Card image cap">
  			<div class="card-body">
    			<h5 class="card-title text-center">Aires Menu</h5>
   				<p class="card-text">Designed and developed a menu as a school project! The menu design was based on the clients specifications. The menu was made with Adobe InDesign.</p>
    			<a href="strongyou.php" class="btn btn-primary col rounded-0">See More</a>
  			</div>
		  </div>
		  <div class="col-lg-10 mt-5 mx-auto text-center">
            <div class="foo  foo-3"><a class="btn btn-dark btn-xl js-scroll-trigger rounded-0" href="index.php">Back</a></div>
          </div>
        </div>
      </div>
    </div>
	  
    <!-- Footer -->
   <?php include 'footer.php';?>

  </body>

</html>
